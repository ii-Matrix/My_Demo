...
//void dma_init(DMA_InitTypeDef *DMA_InitStruct, uint32 cfg)
//{
//
//  //  uint8 BYTEs = (byten == DMA_BYTE1 ? 1 : (byten == DMA_BYTE2 ? 2 : (byten == DMA_BYTE4 ? 4 : 16 ) ) ); //计算传输字节数
//
...
//
//    /* 配置 DMA 通道 的 传输控制块 TCD ( Transfer Control Descriptor ) */
//    DMA_SADDR(DMA_InitStruct->chl) =    (uint32)DMA_InitStruct->sAddr;;                      // 设置  源地址
//    DMA_DADDR(DMA_InitStruct->chl) =    (uint32)DMA_InitStruct->dAddr;                      // 设置目的地址
//    DMA_SOFF(DMA_InitStruct->chl)  =    0x00u;                                   // 设置源地址偏移 = 0x0, 即不变
//    DMA_DOFF(DMA_InitStruct->chl)  =    DMA_InitStruct->dAddrOffset;                              // 每次传输后，目的地址加 BYTEs
//
//  //  DMA_SLAST(DMA_InitStruct->chl) =    DMA_SLAST_SLAST(DMA_InitStruct->sLastAddrAdj);
//  //  DMA_DLAST_SGA(DMA_InitStruct->chl)= DMA_DLAST_SGA_DLASTSGA(DMA_InitStruct->dLastAddrAdj); 
//    
//    DMA_ATTR(DMA_InitStruct->chl)  =    (0
//                         | DMA_ATTR_SMOD(DMA_InitStruct->sMod)               // 源地址模数禁止  Source address modulo feature is disabled
//                         | DMA_ATTR_SSIZE(DMA_InitStruct->sDataWidth)            // 源数据位宽 ：DMA_BYTEn  。    SSIZE = 0 -> 8-bit ，SSIZE = 1 -> 16-bit ，SSIZE = 2 -> 32-bit ，SSIZE = 4 -> 16-byte
//                         | DMA_ATTR_DMOD(0x0)                                     // 目标地址模数禁止
//                         | DMA_ATTR_DSIZE(DMA_InitStruct->dDataWidth)            // 目标数据位宽 ：DMA_BYTEn  。  设置参考  SSIZE
//                        );
//
//    DMA_CITER_ELINKNO(DMA_InitStruct->chl)  = DMA_CITER_ELINKNO_CITER(DMA_InitStruct->majorLoopCnt);   //当前主循环次数
//    DMA_BITER_ELINKNO(DMA_InitStruct->chl)  = DMA_BITER_ELINKNO_BITER(DMA_InitStruct->majorLoopCnt);   //起始主循环次数
//
//    DMA_CR &= ~DMA_CR_EMLM_MASK;                                // CR[EMLM] = 0
//
//    //当CR[EMLM] = 0 时:
//    DMA_NBYTES_MLNO(DMA_InitStruct->chl) =   DMA_NBYTES_MLNO_NBYTES(DMA_InitStruct->minorLoopByteCnt); // 通道每次传输字节数，这里设置为BYTEs个字节。注：值为0表示传输4GB */
//
//    /* 配置 DMA 传输结束后的操作 */
//    DMA_SLAST(DMA_InitStruct->chl)      =   0;                              //调整  源地址的附加值,主循环结束后恢复  源地址
//    DMA_DLAST_SGA(DMA_InitStruct->chl)  =   (uint32)( (cfg & DADDR_KEEPON ) == 0 ? (-DMA_InitStruct->majorLoopCnt)  : 0 ); //调整目的地址的附加值,主循环结束后恢复目的地址或者保持地址
//    DMA_CSR(DMA_InitStruct->chl)        =   (0
//                             | DMA_CSR_BWC(3)               //带宽控制,每读一次，eDMA 引擎停止 8 个周期（0不停止；1保留；2停止4周期；3停止8周期）
//                             | DMA_CSR_DREQ_MASK            //主循环结束后停止硬件请求
//                             | DMA_CSR_INTMAJOR_MASK        //主循环结束后产生中断
//                            );
...
//    /* 配置 DMA 触发源 */
//#if defined(MK60DZ10)
//    DMAMUX_CHCFG_REG(DMAMUX_BASE_PTR, DMA_InitStruct->chl) = (0
//#elif defined(MK60F15)
//    DMAMUX_CHCFG_REG(DMAMUX0_BASE_PTR, DMA_InitStruct->chl) = (0
//#endif
//            | DMAMUX_CHCFG_ENBL_MASK                        /* Enable routing of DMA request */
//            //| DMAMUX_CHCFG_TRIG_MASK                        /* Trigger Mode: Periodic   PIT周期触发传输模式   通道1对应PIT1，必须使能PIT1，且配置相应的PIT定时触发 */
...
//  //  dma_gpio_input_init(SADDR,BYTEs);
//
//    DMA_DIS(DMA_InitStruct->chl);                                    //使能通道CHn 硬件请求
//    DMA_IRQ_CLEAN(DMA_InitStruct->chl);
//
//    /* 开启中断 */
//    DMA_EN(DMA_InitStruct->chl);                                    //使能通道CHn 硬件请求
//    DMA_IRQ_EN(DMA_InitStruct->chl);                                //允许DMA通道传输
//}
//void dma_init2(DMA_CHn CHn, void *SADDR, void *DADDR,  DMA_BYTEn byten, uint32 count, uint32 cfg)
//{
//
//    uint8 BYTEs = (byten == DMA_BYTE1 ? 1 : (byten == DMA_BYTE2 ? 2 : (byten == DMA_BYTE4 ? 4 : 16 ) ) ); //计算传输字节数
//
